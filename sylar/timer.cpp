...
bool Timer::cancel()
{
    if(m_cb)
    {
        SYLAR_LOG_DEBUG(g_logger) << "timer's cb is null!";
        m_cb = nullptr;
    }
...
bool Timer::refresh() 
{
    ...
    if(!m_cb) 
    {
        SYLAR_LOG_INFO(g_logger) << "there is no cb in this timer";
        return false;
    }
...
bool Timer::reset(uint64_t ms, bool from_now) 
{
    if(ms == m_ms && !from_now)
    {
        SYLAR_LOG_INFO(g_logger) << "it is same with the old timer!";
        return true;
    }
...
void TimerManager::listExpiredCb(std::vector<std::function<void()> >& cbs)
{
    ...
    std::set<Timer::ptr, Timer::Comparator>::iterator it;
    {
        ...
        rollover = detectClockRollover(now_ms);
        ...
        it = rollover ? m_timers.end() : m_timers.lower_bound(now_timer);
