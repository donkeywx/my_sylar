...
bool TcpServer::listen(sylar::Address::ptr addr, bool ssl)
{
    std::vector<Address::ptr> addrs;
    std::vector<Address::ptr> fails;
    addrs.push_back(addr);

    return listen(addrs, fails, ssl);
}

bool TcpServer::listen(const std::vector<Address::ptr>& addrs
                        ,std::vector<Address::ptr>& fails
                        ,bool ssl)
{
    m_ssl = ssl;
    for(auto& addr : addrs)
    {
        Socket::ptr sock =  Socket::CreateTCPSocket((Socket::Domain)addr->getFamily());
        if(!sock->listen(addr))
        {
...
void TcpServer::startAccept(Socket::ptr sock)
{
    while(!m_isStop)
    {
        ...
        {
            SYLAR_LOG_ERROR(g_logger) << client << "accept errno=" << errno
